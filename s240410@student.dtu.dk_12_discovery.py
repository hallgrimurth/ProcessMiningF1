...
# from DependencyGraphs import dependency_graph_file, read_from_file
...
class PetriNet:
    def __init__(self):
        ...
        self.initial_marking = {}  # Store the initial marking
    ...
    def is_enabled(self, transition):
        # print(f"Checking if transition {transition} is enabled.")
        for edge in self.f:
            if edge[1] == transition:  # Edge from place to transition
                if self.get_tokens(edge[0]) == 0:
                    # print(f"Transition {transition} is not enabled. No tokens in {edge[0]}.")
                    return False
        print(f"Transition {transition} is enabled.")
        return True

    def fire_transition(self, transition):
        # print(f"Attempting to fire transition {transition}")
        consumed = 0
        produced = 0
        missing = 0
        is_enabled = True
        for edge in self.f:
            if edge[1] == transition:
                ...
                if available_tokens < required_tokens:
                    ...
                    # print(f"Transition {transition} cannot fire. Missing {missing} tokens in {place}.")
                    is_enabled = False
        if not is_enabled:
        ...
        for edge in self.f:
            if edge[1] == transition:
                self.m[edge[0]] -= 1
                consumed += 1
                # print(f"Consumed 1 token from {edge[0]}")
        # Add tokens to output places
        for edge in self.f:
            if edge[0] == transition:
                self.m[edge[1]] = self.m.get(edge[1], 0) + 1
                produced += 1
                # print(f"Produced 1 token in {edge[1]}")
        return consumed, produced, missing
    ...
    def store_initial_marking(self):
        """Stores the current marking as the initial marking."""
        self.initial_marking = self.m.copy()

    def reset_marking(self):
        """Resets the marking to the initial marking."""
...
def fitness_token_replay(log, pn):
    ...
    # Step 1: Count trace frequencies
    trace_frequencies = defaultdict(int)
    for case_id, events in log.items():
        # Sort events by timestamp
        events.sort(key=lambda x: x['time:timestamp'])
        # Get the trace as a tuple of activity names
        trace = tuple(event['concept:name'] for event in events)
        trace_frequencies[trace] += 1
    
    # Step 2: Store the initial marking of the Petri net
    pn.store_initial_marking()
    
    # Step 3: Process each trace
    for trace, frequency in trace_frequencies.items():
        # Reset the marking to the initial state for each trace replay
        pn.reset_marking()
        ...
        # Replay the trace through the Petri net
        for task in trace:
        ...
        # Count the remaining tokens (excluding 'o_W')
        remaining_tokens = sum(pn.get_tokens(place) for place in pn.p if place != 'o_W')
        
        # Weight the results by the trace frequency
        total_remaining += remaining_tokens * frequency
        total_produced += produced * frequency
        total_consumed += consumed * frequency
        total_missing += missing * frequency
    
    # Step 4: Compute fitness using the formula
    if (total_consumed + total_produced) == 0:
        return 0
    if len(trace_frequencies) > 3 :
        return 0.95543
